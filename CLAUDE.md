...
- **One key scheme.** Picker: F, V, S, Esc. Selection toolbar: C, S, E, with Esc clearing the selection. Result tab: P, R, A, X for tools, Enter applies a crop or copies, C and S copy and save, Ctrl+Z undoes. Ctrl+C and Ctrl+S always mirror C and S, and buttons show their key in their label. A new action needs a key that fits, and the key list in the manifest's `default_title`, the README and the store copy must follow.
